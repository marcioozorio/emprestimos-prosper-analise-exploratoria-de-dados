---
title: "Dados de Empréstimos da Prosper - Análise Exploratória de Dados (EDA)"
...
<br/>
Este estudo faz parte do curso de Análise de Dados da [Udacity](http://udacity.com) onde será utilizada a base de dados da empresa de [Prosper](https://www.prosper.com). A Prosper foi fundada em 2005 e foi a primeira plataforma de serviço de empréstimos ponto a ponto (peer-to_peer) dos Estados Unidos com mais de US $ 7 bilhões em empréstimos financiados. 
...
# A coluna ListingCategory contém as informaçoes dos códigos das categorias. Iremos realizar alguns procedimentos para inserir uma nova coluna chamada Category com a descriçao das categorias
...
Conforme as instruções da Udacity, iremos selecionar entre 10 a 15 variáveis para este estudo. O processo de Análise das informações mais relevantes resultou no seguinte mapa mental contendo colunas já existentes, novas colunas que seráo criadas e a Taxa de Juros que esta destacada na cor verde, pois terá um papel importante em nossa Análise.

![](docs/Mapa_Mental_Analise_Prosposcore.png)
<br/>
Portanto, foi seleciondo 13 colunas da nossa base de dados. E com base nestas informaçoes foram adcionados mais 4 variáveis.
...
Primeiro vamos começar realizando uma Análise das distibuições para termos uma idéia geral da base de dados.
...
qplot(data = df, binwidth = 0.4, x = LoanOriginalAmount) + 
  scale_x_log10() +
  theme(text = element_text(size=15), 
        plot.title = element_text(family = "Helvetica", face = "bold", size = (15), hjust = 0.5)) +
  ggtitle("1. Log10 Valor do empréstimo") + 
  labs(x = "Log10 do Valor do empréstimo", y = "Quantidade")

```

Usando a escala de log10 para o valor de empréstimo, não foi possivel identificar exatamente de qual distribuição se trata. Parece ser uma distribuição assimétrica negativa.
...
qplot(data = subset(df, df$StatedMonthlyIncome > 500 & df$StatedMonthlyIncome <= 50000 & !is.na(StatedMonthlyIncome)), x = StatedMonthlyIncome, binwidth = 0.06) + 
  scale_x_log10() +
  theme(text = element_text(size=15), 
        plot.title = element_text(family = "Helvetica", face = "bold", size = (15), hjust = 0.5)) +
  ggtitle("2. Log10 da Renda Mensal") + 
...
qplot(data = df, x = LoanStatus, fill = I("#66b3ff"), col = I("#333333")) + 
  ...
  labs(y = "Quantidade", x = "Status do empréstimo")
...
qplot(data=df, x = df$LoanOriginalAmount, binwidth = 2500, fill = I("#66b3ff"), col = I("#333333"))  +
...
qplot(data = df, x = df$LoanOriginationMonth, fill = I("#66b3ff"), col = I("#333333")) + 
...
ggplot(data = df, aes(x = LoanOriginationMonth, y = LoanOriginalAmount)) +
  geom_bar(stat = 'summary', fun.y = mean, fill = I("#66b3ff"), col = I("#333333"))  + 
  theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (15), hjust = 0.5)) +
...
qplot(x = df$LoanOriginationYear, data=df, fill = I("#66b3ff"), col = I("#333333")) +
  scale_y_continuous(breaks = seq(0,35000, 5000)) +
  theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (15), hjust = 0.5)) +
...
Após 2009 a quantidade de emprestimos tornou a crescer, no entnato, em 2014 o valor volta a cair. Podemos observar também uma pequena quantidade de empréstimos em 2005. Vamos analisar mais de perto esses dois anos: 
...
df_outlayer <- subset(dfCountByYearMonth, LoanOriginationYearMonth <= 2005.12 | LoanOriginationYearMonth >= 2014.01)  
print(df_outlayer)

```

Veja que o ano de 2005 tiveram somente dois meses e a quantidade de emprestimos foram somente 22. Já para o ano de 2014, tivemos somente três meses registrados com emprestimos, mas apesar disso, 2014 tem uma contagem bastante alta, somando 12.172 emprestimos. Vamos continuar a análise.
...
ggplot(data = df, aes(x = LoanOriginationYear, y = LoanOriginalAmount)) +
  geom_bar(stat = 'summary', fun.y = sum, fill = I("#66b3ff"), col = I("#333333")) +
  theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (15), hjust = 0.5)) +
...
qplot(x  = df$LoanOriginationYear, y = df$LoanOriginalAmount) + 
  ...
  geom_boxplot(data=df[df$LoanOriginationYear == "2014",],
                        aes(x = LoanOriginationYear, y = LoanOriginalAmount), fill = "#85e0e0") +
  stat_summary(fun.y = mean,shape=4, col = 'red', geom = 'point') +
  theme(plot.title = element_text(family = "Helvetica", face = "bold", size = (15), hjust = 0.5)) +
...
O "x" vermelho indica a média do valor do empréstimo. O traço horizontal no meio do retângulo é a mediana.
Aqui podemos ver informações estatésticas sobre Distribuição dos emprestimos ao longo dos anos. 
Para a maioria dos anos a média de empréstimos vai até US$ 10.000. De 2010 em diante podemos notar um aumento crescente dos valores a cada ano. Em 2014 vêmos novamente a maior média de todos os anos.
...
df_tree_months <- subset(df, df$LoanOriginationMonth >= "01" & df$LoanOriginationMonth <= "03")
...
qplot(data=df, x = df$MonthlyLoanPayment, binwidth=50, fill = I("#66b3ff"), col = I("#333333")) +
...
dfLoanByOccupation <- subset(dfLoanByOccupation, Occupation != "Other" & Occupation != "Professional")

ggplot(data = dfLoanByOccupation[1:10,], aes(x = Occupation, y = n, colour = Occupation, fill = Occupation)) +
...
As ocupações que tem o maior número de emprestimos são: Programadores de computador, Executivos e Professores.
...
ggplot(aes(x = LoanOriginationYear, y = mean_income, group = 1), data = dfIncomeByYear) + 
...
Podemos notar que em 2012 existe um outlayer. Analisando os detalhes, foi constatado que este mutuário tem a renda mensal de US$ 1750000, mas nao tem a renda comprovada.
...
```{r echo=FALSE, results=FALSE}
# Identificando o outlayer
# não há nada de incomum com esmprétimo. A categoria do empréstimo é Bussiness, ou seja, para negcóios e o valor foi de US 4000.
...
Vamos agora visualizar as informações retirando os outliers, ou seja, vamos filtrar somente os mutuários com renda mensal abaxo de US$ 50000.

```{r echo=FALSE, optipng = '-o7'}
# Analisando renda fora do outlayer (até 50000)

qplot(data = df, x = LoanOriginationYear, y = StatedMonthlyIncome) +
  geom_boxplot(fill = "#e6ecff") + 
  geom_boxplot(data=df[df$LoanOriginationYear == "2005",],
                        aes(x = LoanOriginationYear, y = StatedMonthlyIncome), fill = "#85e0e0") +
  stat_summary(fun.y = mean, shape = 4, col = 'red', geom = 'point') +
...
```{r echo=FALSE, optipng = '-o7'}
df_year_group = group_by(subset(df, df$LoanOriginationYear != "2005"), LoanOriginationYear)
...
```{r fig.width = 10, fig.asp = .75, optipng = '-o7'}

ggplot(aes(x = Category, y = LoanOriginalAmount, colour = Category, fill = Category), data = df) + 
  geom_bar(stat = 'summary', fun.y = mean, show.legend = F) +
...
Respondendo à pergunta número 1, neste gráfico é possível identificar que em em média o maior motivo para a realização do empréstimo são para pagamento de dividas. Seguido de Bebê e Adoção, Negócios e Empréstimos para Casamento.
...
ggplot(aes(x = df$Category, y = df$LoanOriginalAmount),
  ...
  labs(y = "Valor médio do empréstimo", x = "Categoria", color = "Faixa de Renda")

```

O gráfico acima deixa claro que o perfil mais comum de renda de quem realiza empréstimos são mutuários com faixas de renda entre $ 1 a 25000 e os valores dos emprestimos em geral vão somente até US$ 250000 onde a categoria mais utilizada é a para pagamento de dívidas (debt)
A faixa de renda com maior quantidade de emprestimos com valor acima de US 25000 é de 10.000 a 25.000, e bastante utilizado para pagamento de dívidas (debt).
...
ggplot(data = df, aes(x = Term, y = BorrowerRate)) + 
  stat_summary(fun.y = mean, shape = 1, col = 'red', geom = 'bar', color = '#990000', fill = "#990000") +
...
ggplot(aes(x = df$Term, y = df$LoanOriginalAmount), data = df) +
  ...
  labs(y = "Valor do empréstimo", x = "Quantidade de Parcelas", color = "Faixa de Renda")
...
VALOR médio MENSAL DA PARCELA POR FAIXA DE SALÀRIO
```{r fig.width = 8, fig.asp = .62, optipng = '-o7'}
...
ggplot(aes(y = format(dfByIncomeLoan$mean_monthly_loan, scientific = F), x = IncomeRange2, group = 1), data = dfByIncomeLoan) + 
...
ggplot(aes(x = Category, y = BorrowerRate, colour = Category, fill = Category), data = df) + 
  geom_bar(stat = 'summary', fun.y = mean, show.legend = F) +
   theme_bw() +  
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
ggplot(aes(x = df$LoanOriginationYear, y = df$BorrowerRate, group = 1), data = df) +
...
A taxa de juros iniciou em 2005 baixissima, no entanto foram realizados somente 22 empréstimos neste ano. A partir de 2006 houveram grandes aumentos até chegar em 2011, onde se deu inicio de quedas sucessivas da taxa de juros até chegar em 2014 onde iniciou com o juros bem mais baixo com relação aos demais anos. 
...
dfByYearIncomeRange <- arrange(dfByYearIncomeRange, LoanOriginationYear, IncomeRange2)



g28 <- ggplot(aes(x = LoanOriginationYear, y = mean_rate,  color = IncomeRange2, group = IncomeRange2), data = dfByYearIncomeRange) + 
  geom_line(size = .3) +
  geom_point(size = 2) + 
...
ggplot(aes(x = df$EmploymentStatus, y = df$BorrowerRate, group = 1), data = df) +
  geom_line(stat = 'summary', fun.y = mean, color = '#990000', size = 1) +
  geom_point(stat = 'summary', fun.y = mean, color = 'black', size = 2) +
...
```
Respondendo a pergunta 5 sim, mutuários que estão dempregados tem uma taxa de juros bem maior do que os que estão empregados. 
...
df_income_verif_group = group_by(subset(df, df$StatedMonthlyIncome > 0), IncomeVerifiable)
...
Range coeficiênte | Força da correlação
---------------|----------------------
 0,2 < r < 0,4 | Correlação fraca
 0,4 < r < 0,7 | Correlação moderada
 0,7 < r < 0,9 | Correlação forte
*Onde r = coeficiênte de correlação linear*


Vamos calcular o coeficiênte de correlação usando o método de Pearson para encontrar a correlação entre o PropScore e a Taxa de Juros:
...
ggplot(aes(y = BorrowerRate, x = ProsperScore), data = df_score) +
  geom_jitter(alpha = 1/5, fill = I("#ffcc00"), color = I("#191970")) + 
...
Como vimos, o coeficiênte encontrado foi -0,64. Podemos observar que existe uma correlaçao fraca, conforme a referência informada na tabela. Significa que a Taxa de Juros tem uma  correlação fraca com com o Propscore. A taxa de juros provavelmente é influenciado também por outras variáveis, como por exemplo a quantidade de percelas, etc.
...
df_sample <- subset(df, select = c(-Term, -Occupation, -LoanOriginationYearMonth, -Category, -LoanOriginationMonth, -LoanOriginationDate, -LoanStatus, -LoanOriginationYear))

g <- ggpairs(df_sample[sample.int(nrow(df_sample), 5000),], 
             #axisLabels="internal", # Infelizmente a fonte fica muito grande com eixo diagonal
  lower = list(
...
Algumas correlações identificadas são óbvias, e por isso não poderemos considerá-las, como por exemplo o valor mensal a ser pago com o valor total emprestado. Esta relação quase sempre estará relacinoada. Neste caso, o coeficiente de correlação foi bastante alto: 0,93.
...
ggplot(aes(x = ProsperScore, y = LoanOriginalAmount), data = df_score) +
  geom_jitter(alpha=1/5, color = "#191970") +
...
Devido à grande diverença entre os valores disponíveis, tive que usar a funão sqrt para calcularmos a raiz quadrada do valor disponível do cartão de credito e assim deixar o gráfico melhor para visualização:

```{r fig.width = 8, fig.asp = .67, optipng = '-o7'}

ggplot(aes(y = df$BorrowerRate, x = sqrt(df$AvailableBankcardCredit)), data = df) +
  geom_jitter(alpha = 1/5, color = "#191970") +
...
g26 <- ggplot(aes(x = df$LoanOriginationYear, y = df$BorrowerRate, group = 1), data = df) +
  geom_line(stat = 'summary', fun.y = mean, size = .8, color = "#990000") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 13)) + 
...
g30 <- ggplot(aes(x = df$EmploymentStatus, y = df$BorrowerRate, group = 1), data = df) +
  geom_line(stat = 'summary', fun.y = mean, color = '#990000', size = 1) +
  geom_point(stat = 'summary', fun.y = mean, color = 'black', size = 2) +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 13)) +
  scale_y_continuous(breaks=seq(0, 1, 0.01)) + 
