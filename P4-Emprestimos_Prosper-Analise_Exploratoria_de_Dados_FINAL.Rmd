...
# Definições de idioma para uso de ascentuação
...
Como podemos ver a distribuição esta com concentração de valores aproximadamente de 1000 a 5000 e depois tem picos a cada 5000, que indica os valores mais comun de empréstimos. 
...
df_outlayer <- subset(dfCountByYearMonth, LoanOriginationYearMonth >= 2014.01)  
print(df_outlayer)
...
Aqui podemos ver informações estatísticas sobre Distribuição dos emprestimos ao longo dos anos. 
...
As cinco ocupações que tem o maior número de emprestimos são: Programadores de computador, Executivos e Professores, Asistentes Administrativos e Analistas.
Obs.: Foram desconsideros os casos onde a ocupação é igual a Outros (Others) ou Profissional (Professional)
...
```{r echo=FALSE, optipng = '-o7'}
# Analisando renda fora do outlayer (até 7500)
...
A maairia dos empréstimos são utilizados para pagamento de dívidas (debt)
